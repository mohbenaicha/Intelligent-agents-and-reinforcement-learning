...
class Environment():
    ...
    def wumpusInLineOfFire(self):
        int_loc = (type(self.wumpuslocation) == int)
        if not int_loc: return None
        else:
    ...
    def render_np(self):
        ...
        other_feat = []
        hasgold_feat = 1. if self.agent.hasgold else 0.
        other_feat.append(hasgold_feat)
        glitter_feat = 1. if self.isGlitter() else 0.
        other_feat.append(glitter_feat)
        hasarrow_feat = 1. if self.agent.hasarrow else 0.
        other_feat.append(hasarrow_feat)
        scream_feat = 0. if self.agent.wumpusalive else 1.
        other_feat.append(scream_feat)
        other_feat = np.array(other_feat)
        ...
        repeat_feat = [0,0,0,0,0,0,0,0]
        pre_actions = self.agent.previousactions
        pre_locations = self.agent.previouslocations
        if pre_actions:
            if pre_actions[-2:].count(pre_actions[-1]) == len(pre_actions[-2:]) and len(pre_actions[-2:])==2:
                repeat_feat[0]=0.
                repeat_feat[1]=0.
                repeat_feat[2]=1.
                repeat_feat[3]=0.
        ...
        if pre_locations:
            if pre_locations[-5:].count(pre_locations[-1]) == len(pre_locations[-5:]) and len(pre_locations[-5:])==5:
                repeat_feat[4:8]=[0.,0.,1.,0.]
                if pre_locations[-2:].count(pre_locations[-1]) == len(pre_locations[-2:]) and len(pre_locations[-2:])==2:
                    repeat_feat[4:8]=[0.,0.,0.,1.]
                    if pre_locations[-10:].count(pre_locations[-1]) == len(pre_locations[-10:]) and len(pre_locations[-10:])==10:
                        repeat_feat[4:8]=[0.,1.,0.,0.]
                        if pre_locations[-50:].count(pre_locations[-1]) == len(pre_locations[-50:]) and len(pre_locations[-50:])==50:
                            repeat_feat[4:8]=[1.,0.,0.,0.]
                        

        
        
        final_feat = np.concatenate((agentloc_feat.reshape(GH*GW),
                                     visited_feat.reshape(GH*GW),
                                     stench_feat.reshape(GH*GW),
                                     breeze_feat.reshape(GH*GW),
                                     orientation_feat,
                                     other_feat,
                                     repeat_feat
                                     ))
    ...
    def initialize(self, GridWidth,GridHeight,pitProb,allowClimbWithoutGold):
        ...
        self.wumpuslocation = random.choice(coords_list) if self.wumpusalive else None    
